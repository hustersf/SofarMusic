...
public class HighLightActivity extends UIRootActivity {
  ...
  private int[] colors =
      new int[] {Color.parseColor("#DE4A40"), Color.GRAY, Color.parseColor("#DE4A40"), Color.GRAY};
  private int[][] states =
      new int[][] {{android.R.attr.state_selected}, {-android.R.attr.state_selected},
  ...
  private void initTabs() {
    ...
    for (int i = 0; i < tabList.size(); i++) {
      ...
      ll_sub_container[i].setLayoutParams(
          new LinearLayout.LayoutParams(width / tabList.size(), DensityUtil.dp2px(this, 50), 1));


      tv_tab[i] = new TextView(this);
      tv_tab[i].setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
      ...
      iv_tab[i].setLayoutParams(
          new LinearLayout.LayoutParams(DensityUtil.dp2px(this, 30), DensityUtil.dp2px(this, 30)));
  ...
  private void showGuideView() {
    ...
    mHeadBackTv.post(new Runnable() {
      @Override
      public void run() {
        GuideBuilder builder = new GuideBuilder();
        builder.setTargetView(mHeadBackTv)
            .setAlpha(100)
