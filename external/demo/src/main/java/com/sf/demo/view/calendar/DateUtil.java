...
public class DateUtil {
    ...
    public static String getSomeDays(String currentData, int dayNum) {
        ...
        //过去七天
        try {
            c.setTime(DateUtil.dateFormat.parse(currentData));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        c.add(Calendar.DATE, dayNum);
        Date d = c.getTime();
        String day = DateUtil.dateFormat.format(d);
        return day;
    }
     ...
     * @return
     */
    public static List<DateEntity> getWeek(String date) {
     ...
     * @return
     */
    public static List<DateEntity> getMonth(String date) {
        List<DateEntity> result = new ArrayList<>();
        Calendar cal = Calendar.getInstance();
        try {
        ...
        //周天是1，需要补充周一到周六，即6天
        int weekNum = result.get(0).weekNum - 2;
        if (weekNum == -1) {
            weekNum = 6;
        }
    ...
    /**
     * 时区在美国还是中国，1都代表周天
     * DAY_OF_WEEK，单纯就返回的是周几，而不是一周的第几天
     *
     * @param weekNum
     * @return
     */
    ...
    public static boolean isToday(String sdate) {
        ...
        Date today = new Date();
        if (time != null) {
            String nowDate = DateUtil.dateFormat.format(today);
            String timeDate = DateUtil.dateFormat.format(time);
     ...
     * @return
     */
    public static ArrayList<DateEntity> getWeek1(String date){
     ...
     * @return
     */
    public static ArrayList<DateEntity> getMonth1(String date){
        ArrayList<DateEntity> result = new ArrayList<>();
        Calendar cal =Calendar.getInstance();
        try {
            cal.setTime( new SimpleDateFormat("yyyy-MM").parse(date));
