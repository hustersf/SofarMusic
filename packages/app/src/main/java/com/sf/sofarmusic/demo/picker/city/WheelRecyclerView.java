package com.sf.sofarmusic.demo.picker.city;
...
public class WheelRecyclerView extends RecyclerView {
    ...
    private Paint mLabelPaint;  //绘制标签的paint
    ...
    public WheelRecyclerView(Context context, @Nullable AttributeSet attrs) {
        ...
        mLabelPaint.setColor(mSelectTextColor);
        mLabelPaint.setStrokeWidth(mDividerHeight);
        mLabelPaint.setTextSize(mSelectTextSize);
    ...
    private class WheelAdapter extends Adapter<WheelAdapter.WheelHolder> {
        ...
        public WheelHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            WheelHolder holder = new WheelHolder(LayoutInflater.from(getContext()).inflate(R.layout.adapter_item_wheel, parent, false));
    ...
    private class DividerItemDecoration extends ItemDecoration {
        ...
        public void onDrawOver(Canvas c, RecyclerView parent, State state) {
            ...
            //绘制标签
            if (hasLabel) {
                float center = -(mLabelPaint.ascent() + mLabelPaint.descent()) / 2;
                c.drawText(mLabel, endX, (bottomY + topY) / 2 + center, mLabelPaint);
            }
        }
    }
