...
public class PlayerBaseActivity extends BaseActivity
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    mFloatView =
        LayoutInflater.from(getBaseContext()).inflate(R.layout.layout_bottom_music_player, null);
    ...
    receiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(Constant.RefreshProgress)) {
          ...
          updateBottom();
          updateSongList();
          mPlayAdapter.selectSong(mPosition);
  ...
  protected void updateBottom() {
      ...
      for (int i = 0; i < Constant.sPlayList.size(); i++) {
        if (Constant.sPlayList.get(i).isSelected) {
          ...
          if (mType == PlayStatus.LOCAL) {
            ...
            Glide.with(this).load(Constant.sPlayList.get(i).imgUri)
                .error(R.drawable.placeholder_disk_210).into(music_iv);
          } else {
    ...
    if (Constant.sPlayList != null) {
      ...
      mPlayAdapter.setList(PlayDataHolder.getInstance().getSongs());
  ...
  private void initSheetDialog() {
    mSheetDialog = new SofarBottomSheetDialog(baseAt);
    ...
    list_tv.setText("播放列表（" + Constant.sPlayList.size() + "）");
    mPlayAdapter = new PlayListAdapter();
    mPlayAdapter.setList(PlayDataHolder.getInstance().getSongs());
    mPlayAdapter.notifyDataSetChanged();
