...
import com.sf.libskin.listener.ISkinLoader;
...
public class SkinManager implements ISkinLoader {
    ...
    public int getStatusColor() {
        int originId = context.getResources().getIdentifier("head_title_bg_color", "color", context.getPackageName());
        if (mResources == null) {
            return context.getResources().getColor(originId);
        }

        if (SkinConfig.isSPColor) {
            return SkinConfig.skinColor;
        } else {
            int identify = mResources.getIdentifier("head_title_bg_color", "color", skinPackageName);
            return mResources.getColor(identify);
        }
            ...
            protected Resources doInBackground(String... params) {
                try {
                    if (params.length == 1) {
                        ...
                        //  Log.i("TAG", skinPackageName);  //com.sf.skinpackage
    ...
    public int getSPColor(int resId) {
        ...
        //当color是从sp文件中获取时，只改变主题色，其他和默认的颜色一个色调
        if ("themeColor".equals(resName) || "user_area_color".equals(resName) || "head_title_bg_color".equals(resName)) {
    ...
    public void loadSkinFromSp() {
        //随便加载一个纯色主题的皮肤包，因为sp除了themeColor外，其他颜色和纯色主题包一样
        loadSkin("themepink.skin", null);
        //   notifySkinUpdate();   //使得颜色即时生效
    }
}
